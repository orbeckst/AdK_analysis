...
for man in range(len(dcdfile)):
	try:
		...
		t = M.split()
		if t[-1][:1] == '9':
			scal_matrix = numpy.load("%s_scal_ave.pickle" % (t[-1][:5]) )
		else:
			scal_matrix = numpy.load("%s_scal_ave.pickle" % (t[-1][:6]) )
		...
		if t[-1][:1] == '9':
			savefig("%s_scal_ave_cont" % (t[-1][:5]))
		else:
			savefig("%s_scal_ave_cont" % (t[-1][:6]))
		clf()
	except:
		continue	
